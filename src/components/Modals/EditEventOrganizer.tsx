...
interface EventOrganizerFormData {
  name: string;
  email: string;
  phoneNumber: string;
  password?: string;
...
interface EditEventOrganizerProps {
...
const EditEventOrganizer: React.FC<EditEventOrganizerProps> = ({
  onSubmitSuccess,
  id,
  isOpen,
  setIsOpen,
  ...
  const fetchDetail = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${app_urls.getManagerDetails}/${id}`, {
        headers: {
      ...
      if (response.data.status && response.data.data?.user) {
        ...
        if (user.state?._id) {
          setSelectedState(user.state._id);
        }
      } else {
  ...
  const onSubmit = async (data: EventOrganizerFormData) => {
    ...
    try {
      ...
      if (response.data.status) {
        ...
        const errorMsg =
          response.data.message || 'Failed to update event organizer';
        dispatch(addManagerFailure(errorMsg));
        setError(errorMsg);
      }
    } catch (error: any) {
      const message =
        error.response?.data?.message || 'Error updating event organizer';
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            className={clsx(
                              ...
                              errors.phoneNumber
                                ? 'border-red-300 bg-red-50 text-red-900 focus:border-red-500 focus:ring-red-500 dark:border-red-600 dark:bg-red-900/20'
                                : 'border-slate-300 bg-white text-slate-900 focus:border-dark:border-blue-600 focus:ring-blue-600 dark:bg-blue-900 dark:text-white',
                            )}
                      ...
                      >
                        <div className="flex gap-4 mt-2">
                          <label className="inline-flex items-center">
                            <input
                              type="radio"
                              className="form-radio h-4 w-4 text-indigo-600 transition duration-300 ease-in-out"
                              value="true"
                              {...register('active', {
                          ...
                          <Controller
                            ...
                            render={({ field }) => (
                              <select
                                id="stateId"
                                className={clsx(
                                  'w-full rounded-md border py-2.5 pl-10 pr-4 text-sm outline-none appearance-none bg-white dark:bg-slate-700 dark',
                                  errors.stateId
                                    ? 'border-red-300 bg-red-50 text-red-900 focus:border-red-500 focus:ring-red-500 dark:border-red-600 dark:bg-red-900/20'
                                    : 'border-gray-300 dark:border-gray-600 dark:text-white focus:border-blue-500 dark:focus:border-blue-400 focus:ring-blue-500 dark:focus:ring-blue-400',
                                )}
                                value={field.value}
                                onChange={(e) => {
                                  field.onChange(e);
                        ...
                        <div className="relative">
                          ...
                          <textarea
                            ...
                            className={clsx(
                              ...
                              errors.address
                                ? 'border-red-300 bg-red-50 text-red-900 focus:border-red-500 focus:ring-red-500 dark:border-red-600 dark:bg-red-900/20'
                                : 'border-gray-300 bg-white text-slate-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-slate-700 dark:text-white',
                            )}
