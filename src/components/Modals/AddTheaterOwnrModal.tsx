...
interface TheaterOwnerFormData extends ManagerFormData {
...
interface AddTheaterOwnerModalProps {
    onSubmitSuccess : (value: boolean) => void;
}

const AddTheaterOwnerModal: React.FC<AddTheaterOwnerModalProps> = ({onSubmitSuccess}) => {
  ...
  const onSubmit = async (data: TheaterOwnerFormData) => {
    ...
    try {
      ...
      if (response.data.status) {
        ...
        closeModal();
        toast.success('Theatre Owner created successfully!', {
          className: 'z-[99999]',
        });
      } else {
        const errorMsg =
          response.data.message || 'Failed to create theatre owner';
        dispatch(addManagerFailure(errorMsg));
        setError(errorMsg);
      }
    } catch (error: any) {
      const message =
        error.response?.data?.message || 'Error creating theatre owner';
...
export default AddTheaterOwnerModal;
