...
const AddManagerModal: React.FC<ModalFormProps> = () => {
  ...
  const parseDuplicateKeyError = (error: string): string => {
    const fieldMatch = error.match(/index: (\w+)_1/);
    ...
    // Return the appropriate message or a fallback

    setError(fieldMessages[field] || 'A duplicate entry exists.');

    return fieldMessages[field] || 'A duplicate entry exists.';
  };
  ...
  const onSubmit = async (data: ExtendedManagerFormData) => {
    ...
    commonFields.forEach((key) => {
      if (data[key as keyof ExtendedManagerFormData] !== undefined) {
        formData.append(
          key,
          String(data[key as keyof ExtendedManagerFormData]),
        );
      }
    });
