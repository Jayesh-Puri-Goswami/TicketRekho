...
const EditManModal: React.FC<ModalFormProps> = ({
  managerData,
  onSubmitSuccess,
  onClose,
}) => {
  ...
  const onSubmit = async (data: ManagerFormData) => {
    ...
    let normalizedRole = '';
    if (data.role.toLowerCase().includes('event')) {
      normalizedRole = 'eventManager';
    } else if (data.role.toLowerCase().includes('theatre')) {
      normalizedRole = 'theatreManager';
    }
    ...
    const payload = {
      ...
      active: data.status,
      id: data._id, // Include the _id for updating
    };
    ...
    try {
      const response = await axios.post(`${urls.editMangerProfile}`, payload, {
        headers: {
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="p-6.5 grid grid-cols-1 md:grid-cols-2 gap-x-6">
            ...
            {success && (
              <p className="text-green-500">Manager updated successfully!</p>
            )}
          </div>
        </form>
      </div>
