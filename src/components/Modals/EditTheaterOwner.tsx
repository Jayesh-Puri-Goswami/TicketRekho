...
interface TheaterOwnerFormData {
  name: string;
  email: string;
  phoneNumber: string;
  password?: string;
...
interface EditTheaterOwnerProps {
...
const EditTheaterOwner: React.FC<EditTheaterOwnerProps> = ({
  onSubmitSuccess,
  id,
  isOpen,
  setIsOpen,
  ...
  const onSubmit = async (data: TheaterOwnerFormData) => {
    ...
    fields.forEach((key) => {
      if (data[key as keyof TheaterOwnerFormData] !== undefined) {
        formData.append(key, String(data[key as keyof TheaterOwnerFormData]));
      }
    });
    ...
    try {
      const response = await axios.put(`${app_urls.updateClient}/${id}`, formData, {
        headers: {
          Authorization: `Bearer ${currentUser.token}`,
      ...
      if (response.data.status) {
        ...
        closeModal();
        toast.success('Theatre Owner updated successfully!', {
          className: 'z-[99999]',
        });
        onSubmitSuccess(true);
      } else {
        const errorMsg = response.data.message || 'Failed to update theatre owner';
        dispatch(addManagerFailure(errorMsg));
        setError(errorMsg);
      }
    } catch (error: any) {
      const message = error.response?.data?.message || 'Error updating theatre owner';
...
export default EditTheaterOwner;