...
const GrabABitesTable: React.FC = () => {
  const { id } = useParams(); 
 ...
 // console.log("id",id);
 // const data = {theatre:id, foodType:''};
  ...
  const fetchSellers = (page: number, limit: number) => {
    setLoading(true);
    axios
      .get(`${Urls.getGrabABiteListwithpagination}?theatre=${id}&page=${page}&limit=${limit}`, {
        headers: {
  ...
  const toggleStatus = async (id: string, currentStatus: boolean) => {
    try {
      ...
      if (response.data.status) {
        ...
        toast.error('Failed to change status of grab a bite');
      }
    } catch (error: any) {
      console.error(
        'Error to change status a grab a bite:',
      ...
      if (error.response?.status === 401) {
        ...
        toast.error('An error occurred while to change status of a grab a bite.');
      }
    }
  };
        ...
        <table className="w-full table-auto">
          <thead>
            <tr className="bg-gray-2 text-left dark:bg-meta-4">
              ...
              <th
                className="min-w-[150px] py-4 px-4 font-medium text-black dark:text-white cursor-pointer text-center"
                onClick={() => handleSort('status')}
              >
...
export default GrabABitesTable;
